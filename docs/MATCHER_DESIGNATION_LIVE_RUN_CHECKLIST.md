...
All app-side logic is already verified headless (34 new assertions plus the
685-assertion main baseline re-run clean = 719 total). This checklist confirms
...
- `AY-00207` — 1916-D Mercury Dime, `Designation` blank, `CoinID =
  C-1916-D-10C-01` (the plain row). Its base key has exactly two catalog
  rows: plain `C-1916-D-10C-01` + FB `C-1916-D-10C-02`. **The clean
  blank↔FB demonstrator.**
- `AY-00240` — 1945-D Mercury Dime, `Designation = FB`, `CoinID =
  C-1945-D-10C-02` (the FB row — already correctly linked). Base key: plain
  `C-1945-D-10C-01` + FB `C-1945-D-10C-02`. **The clean FB demonstrator.**
...
## B. Blank-Designation dime: FB narrowing + the re-link confirm (`AY-00207`)

5. Catalog → find `AY-00207` (1916-D Mercury Dime) → **Edit**. (Sign-in
...
7. **Verify:** a dialog titled **"Confirm catalog re-link"** appears, reading
   `CoinID: C-1916-D-10C-01 → C-1916-D-10C-02`. **The ambiguous "pick one"
...
8. Tap **Cancel**. **Verify in the copy workbook:** `AY-00207`'s row is
   **unchanged** — `Designation` still blank, `CoinID` still
   `C-1916-D-10C-01`. Nothing was written (the whole save is gated behind the
   confirm).
9. Save again, this time tap **Confirm**. **Verify in the copy workbook:**
   `AY-00207` now has `Designation = FB` **and** `CoinID = C-1916-D-10C-02`.
   Both landed.
10. **Restore it** (so the copy isn't left changed and to test the reverse
    ...
    **Confirm** the re-link. **Verify:** the dialog read
    `CoinID: C-1916-D-10C-02 → C-1916-D-10C-01`, and the row is back to blank
    Designation / `C-1916-D-10C-01`. This is the blank-as-value rule
    selecting the plain row.

---

## C. FB dime: blank selects plain, FB selects FB (`AY-00240`)

11. Catalog → `AY-00240` (1945-D Mercury Dime, already `Designation = FB`) →
    **Edit**.
...
13. **Restore:** reopen Edit, set **Designation** back to `FB`, Save,
    **Confirm**. Dialog reads `C-1945-D-10C-01 → C-1945-D-10C-02`; row back to
    `FB` / `C-1945-D-10C-02`.
14. **Sanity — no spurious dialog when nothing re-links:** reopen `AY-00240`
...
23. Report back what passed and anything that didn't. Most useful to me:
    ...
    - Whether the plain-vs-FB **picker is gone** for the clean cases
      (`AY-00207` §B, `AY-00240` §C) while the plain-vs-plain picker
      **remains** for `AY-00208` (§E).

---

## Known limitations of this pass — not bugs

- **Blank-Designation FB dimes resolve to the plain row.** An owned Mercury
  dime that is physically Full Bands but not yet recorded `FB` in
  `All.Designation` silently resolves to the plain catalog row instead of
  surfacing the picker. Deliberate, accepted (it's what stops the 78 FB rows
  from flooding the Docket). Logged as a ParkingLot item — "Physical FB check
  pass on owned Mercury dimes" (Data / Medium). See CLAUDE.md session log.
